...
class Minecraft extends MX_Controller {
    ...
    private function priceplan_init()
    {
        ...
        if($this->module->active_service == 'groups')
        {
			...
			$firstarray_group = array_keys($this->priceplan_raw['sms']['groups']);
            $group = array_pop($firstarray_group);
    ...
    public function index()
    {
        ...
        if($this->input->post('submit'))
        {
            ...
            else 
            {
                ...
                $response = $this->{$pay_method.'code'}->code_response_wsearch($pay_code, $post_price);
