...
class Ui
{
    ...
    public function sms_sendtext($pricelist, $default_prices)
    {
        ...
        else
        {
            ...
            $firstarray_country = array_keys($this->CI->config->item('countries'));
            $first_country = array_pop($firstarray_country);
            
            // Default key from current list
			$firstarray_price = array_keys($pricelist);
            $data['first_key'] = array_pop($firstarray_price);
        }
		...
		$data['price'] = str_replace($data['curr_currency'], '', $data['price']);
